// index.js - VixStream proxy (safe: serves public/watch.html for /watch)
...
// /watch route now serves a static HTML file: public/watch.html
app.get("/watch/:type/:id/:season?/:episode?", async (req, res) => {
  // Ensure public/watch.html exists
  const watchPath = path.join(__dirname, "public", "watch.html");
  if (!fs.existsSync(watchPath)) {
    return res.status(500).send("Missing public/watch.html. Place the client HTML you provided into public/watch.html");
  }

  // Optionally you can pass server-side values via query params or a small JSON endpoint.
  // For simplicity the client will call the local /hls endpoints to fetch metadata.
